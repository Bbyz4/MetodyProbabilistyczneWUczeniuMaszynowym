...
def PerformTests():
    ...
    results = {a : [] for a in np.arange(1000, 51000, 1000)}
    ...
    for z in range(NUMBER_OF_TOTAL_TRIES):
        ...
        decisiveFunctionList = logistic_regression.GetDecisiveFunctionsForComparation(trainingSet, 50000, 1000, 0.5)
        
        for k in range(len(decisiveFunctionList)):
            results[(k+1)*1000].append(CalculateDecisiveFunctionLoss(testingSet, decisiveFunctionList[k]))
...
def PlotResults(results):
    ...
    plt.ylabel("Calculated loss function")
